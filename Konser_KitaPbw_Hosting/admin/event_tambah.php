...
if (isset($_POST['simpan'])) {
    ...
    if (!empty($nama_file)) {
        ...
        if (in_array($ekstensi, $ekstensi_diperbolehkan) === true) {
            if ($ukuran_file < 5242880) {
                ...
                if (move_uploaded_file($tmp_file, '../assets/' . $nama_file_baru)) {
                    ...
                    $insert_event = mysqli_query($conn, "
                        INSERT INTO event (nama_event, deskripsi, tanggal, genre, lokasi, poster)
                        VALUES ('$nama_event', '$deskripsi', '$tanggal', '$genre', '$lokasi', '$nama_file_baru')
                    ");
                    
                    if ($insert_event) {
                        $id_event_baru = mysqli_insert_id($conn);
                        
                        $gagal_kategori = false;
                        foreach ($arr_nama_kategori as $index => $nama_kat) {
                            ...
                            if (!empty($nama_kat_clean)) {
                                $insert_kategori = mysqli_query($conn, "
                                    INSERT INTO kategori (id_event, nama_kategori, harga, kuota_total, kuota_sisa)
                                    VALUES ('$id_event_baru', '$nama_kat_clean', '$harga_clean', '$kuota_clean', '$kuota_clean')
                                ");
                                if (!$insert_kategori) {
                                    $gagal_kategori = true;
                                }
                        ...
                        if (!$gagal_kategori) {
                            $sukses = "Event dan semua kategori tiket berhasil diterbitkan.";
                        } else {
                            $error = "Event tersimpan, tetapi beberapa kategori tiket gagal dimasukkan.";
                        }
                    } else {
                        $error = "Gagal menyimpan data event ke database.";
                    }
                } else {
                    $error = "Gagal mengunggah file gambar poster.";
                }
            } else {
        ...
        $error = "Wajib mengunggah file poster event.";
    }
}
?>
...
<head>
    ...
    <title>Tambah Event Baru - KonserKita</title>
...
<body class="bg-[#FDFBF7] text-[#1A2E26] antialiased min-h-screen flex flex-col md:flex-row">
    ...
    <main class="flex-1 p-4 md:p-10 space-y-6 overflow-x-hidden">
        <div>
            ...
            <h2 class="text-2xl md:text-3xl font-black">Tambah Event Baru</h2>
            <p class="text-xs md:text-sm text-[#1A2E26]/60">Isi formulir berikut untuk mempublikasikan konser musik baru beserta multi-kategori tiket.</p>
        </div>
            ...
            <form method="POST" action="" enctype="multipart/form-data" class="space-y-6">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5 md:gap-6">
                    <div class="space-y-2">
                        ...
                        <input type="text" name="nama_event" required placeholder="Contoh: Jakarta Pop Showcase" class="w-full bg-[#FDFBF7] border border-[#EAE3D2] rounded-xl px-4 py-3 text-sm font-medium focus:outline-none focus:border-[#D4A373]">
                    </div>

                    <div class="space-y-2">
                        <label class="block text-xs font-bold uppercase tracking-wider text-[#1A2E26]/60">Lokasi / Venue</label>
                        <input type="text" name="lokasi" required placeholder="Contoh: Tennis Indoor" class="w-full bg-[#FDFBF7] border border-[#EAE3D2] rounded-xl px-4 py-3 text-sm font-medium focus:outline-none focus:border-[#D4A373]">
                    </div>

                    <div class="space-y-2">
                        <label class="block text-xs font-bold uppercase tracking-wider text-[#1A2E26]/60">Tanggal Pelaksanaan</label>
                        <input type="date" name="tanggal" required min="<?php echo date('Y-m-d'); ?>" class="w-full bg-[#FDFBF7] border border-[#EAE3D2] rounded-xl px-4 py-3 text-sm font-medium focus:outline-none focus:border-[#D4A373]">
                    </div>
                    ...
                    <div class="space-y-2 md:col-span-2">
                        <label class="block text-xs font-bold uppercase tracking-wider text-[#1A2E26]/60">Poster Event (Gambar)</label>
                        <input type="file" name="poster" required class="w-full bg-[#FDFBF7] border border-[#EAE3D2] rounded-xl px-4 py-2.5 text-xs font-medium focus:outline-none focus:border-[#D4A373] file:mr-4 file:py-1.5 file:px-3 file:rounded-xl file:border-0 file:text-xs file:font-bold file:bg-[#1A2E26] file:text-white file:cursor-pointer file:hover:bg-[#1A2E26]/90">
                    </div>
                </div>

                <div class="space-y-2">
                    <label class="block text-xs font-bold uppercase tracking-wider text-[#1A2E26]/60">Deskripsi Event</label>
                    <textarea name="deskripsi" rows="4" required placeholder="Masukkan detail acara..." class="w-full bg-[#FDFBF7] border border-[#EAE3D2] rounded-xl px-4 py-3 text-sm font-medium focus:outline-none focus:border-[#D4A373] resize-none"></textarea>
                </div>

                <div class="border-t border-[#EAE3D2]/60 pt-6 space-y-4">
                    <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3">
                        <h3 class="font-extrabold text-base text-[#1A2E26] flex items-center gap-2"><i class="fa-solid fa-ticket text-[#D4A373]"></i> Pengaturan Kategori Tiket</h3>
                    ...
                    <div id="container-kategori" class="space-y-4">
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 bg-[#FDFBF7] p-4 rounded-2xl border border-[#EAE3D2] items-end relative row-kategori">
                            <div class="space-y-2">
                                <label class="block text-[10px] font-bold uppercase tracking-wider text-[#1A2E26]/60">Nama Kategori</label>
                                <input type="text" name="nama_kategori[]" required placeholder="Contoh: VIP 1" class="w-full bg-white border border-[#EAE3D2] rounded-xl px-3 py-2.5 text-xs font-medium focus:outline-none focus:border-[#D4A373]">
                            </div>
                            <div class="space-y-2">
                                <label class="block text-[10px] font-bold uppercase tracking-wider text-[#1A2E26]/60">Harga Tiket (Rp)</label>
                                <input type="number" name="harga[]" required min="0" placeholder="Contoh: 1500000" class="w-full bg-white border border-[#EAE3D2] rounded-xl px-3 py-2.5 text-xs font-medium focus:outline-none focus:border-[#D4A373]">
                            </div>
                            <div class="space-y-2">
                                <label class="block text-[10px] font-bold uppercase tracking-wider text-[#1A2E26]/60">Kuota Total</label>
                                <input type="number" name="kuota_total[]" required min="0" placeholder="Contoh: 100" class="w-full bg-white border border-[#EAE3D2] rounded-xl px-3 py-2.5 text-xs font-medium focus:outline-none focus:border-[#D4A373]">
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="pt-4">
                    <button type="submit" name="simpan" class="w-full md:w-auto md:float-right bg-[#1A2E26] hover:bg-[#1A2E26]/90 text-white px-8 py-3.5 rounded-xl font-bold text-sm tracking-wide transition-all shadow-md">
    ...
    <script>
        ...
        document.getElementById('btn-tambah-kategori').addEventListener('click', function() {
            ...
            newRow.innerHTML = `
                <div class="space-y-2">
                    <label class="block text-[10px] font-bold uppercase tracking-wider text-[#1A2E26]/60">Nama Kategori</label>
                    <input type="text" name="nama_kategori[]" required placeholder="Contoh: Reguler" class="w-full bg-white border border-[#EAE3D2] rounded-xl px-3 py-2.5 text-xs font-medium focus:outline-none focus:border-[#D4A373]">
                </div>
                <div class="space-y-2">
                    <label class="block text-[10px] font-bold uppercase tracking-wider text-[#1A2E26]/60">Harga Tiket (Rp)</label>
                    <input type="number" name="harga[]" required min="0" placeholder="Contoh: 500000" class="w-full bg-white border border-[#EAE3D2] rounded-xl px-3 py-2.5 text-xs font-medium focus:outline-none focus:border-[#D4A373]">
                </div>
                <div class="space-y-2 relative">
                    <label class="block text-[10px] font-bold uppercase tracking-wider text-[#1A2E26]/60">Kuota Total</label>
                    <input type="number" name="kuota_total[]" required min="0" placeholder="Contoh: 200" class="w-full bg-white border border-[#EAE3D2] rounded-xl px-3 py-2.5 text-xs font-medium focus:outline-none focus:border-[#D4A373]">
